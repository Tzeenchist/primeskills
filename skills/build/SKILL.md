...
2. Run the suite once before touching anything → **verify:** you know which
   failures were already there
3. Restate the task as a verifiable goal → **verify:** it names a command or an
...
7. **Green.** Write the least code that passes → **verify:** `verify` reports PASS
8. **Refactor.** Clean only what you just wrote → **verify:** tests still PASS,
   and the diff contains no unrelated change
9. Every 2–3 steps, commit atomically. Stage intentional files only, never
   `git add -A` → **verify:** `git diff --staged` shows exactly what you meant
   to include (G11)
10. Repeat from 6 until the acceptance criteria are met → **verify:** re-read
