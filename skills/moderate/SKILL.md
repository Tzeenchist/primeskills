...
out, and after one, when part of it has gone stale. Not for picking one next
task — that is a read, not a skill.
...
- The queue's own conventions beat yours: two queues on one machine are kept
  two ways, and the file in front of you says which.
...
3. Sort into ready, dependent, blocked, owner-deferred and owner-only. Blocked
   names its condition and who lifts it; dependent names what it waits on, by
   id; owner-deferred is a decision, which closing the dependency does not
   lift; owner-only needs a credential or an act no agent here has. Unanchored
   and conflicting entries go last, and are said to → **verify:** nothing sits
...
5. Group by contact surface — the files a task opens, not its theme. Entries
   sharing one go adjacent, so it opens once
...
6. Order: ready first, dependencies before dependents, surface groups whole.
   ...
   place at the head. Run `eng` over the first two or three; a wrong-at-root
   verdict takes that entry off the head → **verify:** the top three carry no
   open dependency and no external condition
7. Snapshot what you are about to move, untracked work included (G11): the
   queue is a tracked file other agents also write, and `git checkout` restores
   only what was committed. Re-read the file immediately before the patch — the
   base changed since step 1 means a second writer is in it, so stop and say so.
   Then write, each move carrying its reason in one clause. A file that mirrors
   itself — a summary table beside the cards — moves every mirror together or
   none → **verify:** the diff is move-only, the ids before and after are one
   multiset, and no entry's text is rewritten

## Stop conditions
- The queue file already has uncommitted changes: stop, at the start and again
  before writing. Someone else is in the file, and a clean check that only ran
  once is how their work disappears.
...
- More live entries than the reading budget allows (G3): work in batches and
  never hand back an unchecked tail as a finished order. Name what you did not
  reach.
