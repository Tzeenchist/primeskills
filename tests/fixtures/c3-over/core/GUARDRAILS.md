...
- **G9 Destructive commands.** Recursive deletes, history rewrites, dropped or
  ...
  are not asked to memorise it. Necessary, never sufficient: see G8.
...
## Loops
- **G12 Circuit breaker.** One counter per problem, owned by whoever runs the
  ...
  At three: restore the G11 snapshot, report what each attempt ruled out, ask.
...
- **G14 Checkpoints.** Every 2–3 completed steps, record status and commit
