...
контекста и без телеметрии. Точный состав и вес печатает
`python3 bin/primeskills-status`; там три разные цифры, и путать их не надо —
постоянно в контексте лежат только `core/` и описания скиллов, тело приходит
при вызове. Проход `brief → plan → build → verify → vet → land` против ≈ 92 000
слов у одного прохода gstack — замер в `bench/RESULT.md`.
